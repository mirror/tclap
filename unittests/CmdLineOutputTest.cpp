...
struct BasenameCase {
  BasenameCase(const char * name, const char * basename) : name(name), basename(basename) {}
  const char * name;
  const char * basename;
};

struct RemoveSuffixCase {
  RemoveSuffixCase(const char *str, const char *suffix, const char * want) : str(str), suffix(suffix), want(want) {}
...
void TestRemoveSuffix(Testing &t) {
  ...
  for (unsigned i = 0; i < sizeof(tests)/sizeof(tests[0]); i++) {
    ...
    if (got != test.want) {
      ERROR(t, "removeSuffix(" << test.str << ", " << test.suffix << "), got \"" << got << "\", want \"" << test.want << '"');
    }
  }
}
